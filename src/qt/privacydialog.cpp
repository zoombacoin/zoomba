                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZBA ought to be enough for anybody." - Bill Gates, 2017
    ui->zZBApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZBASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZBA->setEnabled(false);
        ui->pushButtonMintzZBA->setToolTip(tr("zZBA is currently disabled due to maintenance."));

        ui->pushButtonSpendzZBA->setEnabled(false);
        ui->pushButtonSpendzZBA->setToolTip(tr("zZBA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZBApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZBA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZBA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZBA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZBA in ") +
...
void PrivacyDialog::on_pushButtonSpendzZBA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZBA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZBA
        sendzZBA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZBA
    sendzZBA();
}

void PrivacyDialog::on_pushButtonZZBAControl_clicked()
{
    ZZBAControlDialog* zZBAControl = new ZZBAControlDialog(this);
    zZBAControl->setModel(walletModel);
    zZBAControl->exec();
}

void PrivacyDialog::setZZBAControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZBASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZBA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Zoomba Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zZBApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZBA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ZBA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZBA</b>";
    ...
    // use mints from zZBA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZZBAControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZZBAControlDialog::GetSelectedMints();
    }
    ...
    ZZBAControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZBA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ZBA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZBA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZBA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZBA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZBA "));
    ui->labelzZBAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZBASyncStatus->setVisible(fShow);
}
