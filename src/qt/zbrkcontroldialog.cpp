...
std::list<std::string> ZZBAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZZBAControlDialog::listMints;

ZZBAControlDialog::ZZBAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZBAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZZBAControlDialog::~ZZBAControlDialog()
{
    delete ui;
}

void ZZBAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZBAControlDialog::updateList()
{
...
void ZZBAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZBAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZZBA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZBAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZZBAControlDialog::GetSelectedMints()
{
...
void ZZBAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
